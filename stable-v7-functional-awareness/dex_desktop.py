...
PET_W = 188
PET_H = 132
DEX_VERSION = "art-v2-grounded-motion"
...
class DexDesktop:
    def __init__(self):
        ...
        self.menu.add_command(label="Pause roaming", command=self.pause_roaming)
        self.menu.add_separator()
    ...
    def apply_activity_reaction(self):
        activity = self.activity_monitor.snapshot()
        exe = activity["exe"]
        ...
        if cpu is not None and cpu >= 85:
            self.pose = "patrol"
            self.current_reaction = "cpu_spike"
            return True

        if exe == "excel.exe":
            self.pose = "sit"
            self.current_reaction = "excel_disappointed"
            return True

        if exe == "powerpnt.exe":
            self.pose = "sleep" if active_seconds >= 20 * 60 else "sit"
            self.current_reaction = "powerpoint_bored"
            return True

        if exe in {"teams.exe", "ms-teams.exe", "zoom.exe"}:
            if active_seconds >= 60 * 60:
                self.pose = "leash"
                self.item = "leash"
            else:
                self.pose = "sit"
            self.current_reaction = "meeting_watch"
            return True
        ...
        if exe in engineering_exes or any(token in title for token in engineering_titles):
            self.pose = "patrol"
            self.current_reaction = "supervising"
            return True
    ...
    def draw_sprite_frame(self):
        ...
        self.canvas.create_image(2 + stride, 2, image=frames[index], anchor="nw")
        self.draw_life_overlays(pose)
        return True

    def draw_life_overlays(self, pose):
        now = time.time()
        facing_right = pose.endswith("_left")
        blink = now % 6.5 > 6.25

        if facing_right:
            eye_box = (133, 35, 141, 39)
        else:
            eye_box = (45, 35, 53, 39)
